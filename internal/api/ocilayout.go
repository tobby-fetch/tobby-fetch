...
// layoutPlanResponse is the side-effect-free projection: what the export
// would contain, how many bytes it would write, and the size of the
// largest single file — the numbers the pre-flight compares with free
// space and with a target filesystem's per-file limit (FR-055).
//
// Named for its subject rather than for its verb: this package also holds
// the synchronization plan of R-04, and "the plan response" is ambiguous
// the moment there are two of them.
type layoutPlanResponse struct {
