...
// runSplit executes the CLI keeping stdout and stderr apart, which is the
// contract these commands rely on: the report goes to stdout so it can be
// piped into a tool, the operation's own log narration to stderr. The
// shared run() helper merges the two, and a merged stream is not
// machine-readable.
