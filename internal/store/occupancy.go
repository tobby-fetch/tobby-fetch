...
// PhysicalBytes is the on-disk size of the store's blob tree: real,
// deduplicated bytes, unlike the logical sizes of RepoInfo. It is the
// figure the dashboard tile shows and the one the occupancy threshold is
// compared against — one measurement, so the banner and the tile can never
